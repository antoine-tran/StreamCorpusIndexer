	...
	Reducer<SplitEmittedTerm, MapEmittedPostingList, Object, Object>
{
	...
	{	
		...
		RunData = new DataOutputStream(
				Files.writeFileStream(p.toString())
				);
	...
	private static void recursiveMkdir(Path p) {
		Path par = p.getParent();
		if (par == null) {
			if (!Files.exists(p.toString()))
				Files.mkdir(p.toString());
			return;
		}
		else if (Files.exists(par.toString())) {
			if (!Files.exists(p.toString()))
				Files.mkdir(p.toString());
		}
		else recursiveMkdir(par);
	}
	...
	/** Finish up the map processing. Forces a flush, then writes out the final run data   Flush forced*/
	protected void closeMap() throws IOException
	{
		...
		{
			currentIndex.addIndexStructure("document-factory", FieldDocumentIndexEntry.Factory.class.getName(),
	...
	protected String[] MapIndexParents = null;
	...
	protected LinkedList<MapData> loadRunData() throws IOException 
	{
		...
		// Change the search into recursive mode
		final List<FileStatus> files = new ArrayList<FileStatus>();
		
		recursiveListStatus(FileSystem.get(jc),
		...
		for (FileStatus file : files) 
		{
			...
			if (mutipleIndices && partitionChecker.calculatePartition(tempHRD.getSplitnum(), 
					jc.getNumReduceTasks()) != thisPartition)
				continue;
	...
	private void recursiveListStatus(FileSystem fs, Path p, PathFilter pf, List<FileStatus> output) throws IOException {
		...
		FileStatus[] subDir = fs.listStatus(p);
		for (FileStatus status : subDir) {
			if (status.isDir()) {
				String subname = status.getPath().getName();
	...
	protected void mergeDocumentIndex(Index[] src, int numdocs) throws IOException
	{
		...
		try{ 
			...
			{
				final Iterator<DocumentIndexEntry> docidInput = 
						(Iterator<DocumentIndexEntry>)srcIndex.getIndexStructureInputStream("document");
	...
	protected void closeReduce() throws IOException {
		...
		currentIndex.addIndexStructure(
				...
				+ "java.lang.String,org.terrier.structures.DocumentIndex,java.lang.Class", 
		...
		if (FieldScore.FIELDS_COUNT > 0)
			currentIndex.addIndexStructure("lexicon-valuefactory", FieldLexiconEntry.Factory.class.getName(),
					"java.lang.String", "${index.inverted.fields.count}");
