...
public class ApplicationSetup {
	...
	*/
	...
	static {
		// in a dedicated server, must register hdfs scheme first
		try {
			new HadoopPlugin().initialise();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	{		
		...
		try {
			...
			else
			{
				...
				// TODO: Terrier requires access to local file systems in the cluster nodes. This is impossible in 
				// my case, so I have to hard-code the path here
			...
			FSDataInputStream fis = fs.open(new Path(propertiesFile));
			
			configure(new BufferedInputStream(fis));
			fis.close();
